...
export default async function ProjectReturnsPnLPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const returnsItems = items.filter(i => i.category === 'RETURNS');
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 md:p-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 className="text-3xl font-bold text-gray-900">Profit & Loss (Returns)</h1>
        ...
        <div className="rounded-2xl bg-white p-8 shadow-sm border border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-lg font-semibold text-gray-900">Net Returns Value</h2>
                    <p className="text-sm text-gray-500 mt-1">Value recovered from materials returned from site.</p>
                </div>
            </div>
            <div className="text-4xl font-bold">
                 {formatVariance(summary.returnsValueMinor)}
            </div>
        </div>

        {/* Detailed Table */}
        <div className="space-y-6">
            <PnLVarianceTable title="Site Returns" description="Value recovered from site returns" items={returnsItems} />
        </div>
    </div>
  );
}
