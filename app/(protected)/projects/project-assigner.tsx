...
export function ProjectAssigner({ projectId, initialAssigneeId, projectManagers, variant = 'card' }: ProjectAssignerProps) {
  ...
  return (
    <div className="mt-3 bg-white p-4 rounded-xl border border-gray-100 shadow-sm dark:bg-gray-800 dark:border-gray-700">
      ...
      <div className="flex items-center gap-2">
        <select
          className="flex-1 block w-full rounded-lg border-gray-200 bg-gray-50 text-sm focus:border-green-500 focus:bg-white focus:ring-2 focus:ring-green-500/20 dark:border-gray-600 dark:bg-gray-900 dark:text-white"
          value={selectedId}
          onChange={(e) => setSelectedId(e.target.value)}
          disabled={isPending}
        >
        ...
        <button
          onClick={handleAssign}
          disabled={isPending || !selectedId || !isChanged}
          className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
        >
