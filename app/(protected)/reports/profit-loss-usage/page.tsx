...
export default async function GlobalUsagePnLPage() {
  ...
  // Filter for Usage Only
  const usageItems = globalItems.filter(i => i.category === 'USAGE');
  const totalVarianceMinor = globalSummary.usageVarianceMinor;
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 md:p-8 space-y-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 pb-6">
            <div>
                 <div className="flex items-center gap-2 mb-2">
                    <span className="inline-flex items-center rounded-md bg-amber-100 px-2 py-1 text-xs font-medium text-amber-700 ring-1 ring-inset ring-amber-700/10">
                        Global Report
                    </span>
                 </div>
                ...
                <p className="text-gray-500 mt-1">
                    Analysis of material over-usage and wastage across {projectCount} projects.
                </p>
            </div>
        ...
        <div className="rounded-2xl bg-white p-8 shadow-sm border border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Cost of material used in excess of quoted quantities.</p>
                </div>
            </div>
        ...
        <div className="space-y-6">
            <GlobalPnLTable title="Usage Variances" items={usageItems} pageSize={15} />

            {usageItems.length === 0 && (
                <div className="text-center py-12 bg-white rounded-xl border border-gray-200 border-dashed">
                    <p className="text-gray-500">No material over-usage alerts found.</p>
                </div>
            )}
        </div>
    </div>
