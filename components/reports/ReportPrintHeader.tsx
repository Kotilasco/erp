...
export default function ReportPrintHeader({ title, subTitle }: { title: string; subTitle?: string }) {
  return (
      ...
      <div className="flex flex-col md:flex-row justify-between items-start">
        ...
        <div className="flex flex-col items-end text-right gap-1 text-sm text-blue-900 mt-4 md:mt-0">
          ...
          <div className="mt-8 text-right">
             <h1 className="text-3xl font-bold text-gray-500 uppercase tracking-widest">{title}</h1>
             {subTitle && (
                <p className="text-sm text-gray-500 uppercase tracking-wide mt-1">{subTitle}</p>
             )}
          </div>
        </div>
      </div>
